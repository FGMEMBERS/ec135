...
leftBackDoor = aircraft.door.new( "/sim/model/ec135/door-positions/leftBackDoor", 4, 0 );
rightBackDoor = aircraft.door.new( "/sim/model/ec135/door-positions/rightBackDoor", 4, 0 );
...
aircraft.light.new("sim/model/ec135/lighting/strobe-top", [0.05, 1.00], strobe_switch);
aircraft.light.new("sim/model/ec135/lighting/strobe-bottom", [0.05, 1.03], strobe_switch);

# beacons ===========================================================
var beacon_switch = props.globals.initNode("controls/lighting/beacon", 1, "BOOL");
aircraft.light.new("sim/model/ec135/lighting/beacon-top", [0.62, 0.62], beacon_switch);
aircraft.light.new("sim/model/ec135/lighting/beacon-bottom", [0.63, 0.63], beacon_switch);
...
var Doors = {
	new: func {
		...
		m.active = 0;
		m.list = [];
		foreach (var d; props.globals.getNode("sim/model/ec135/doors").getChildren("door"))
...
var fuel = {
	init: func {
		...
		me.pump_capacity = 6.6 * L2GAL / 60; # same pumps for transfer and supply; from ec135 p2: 6.6 l/min older versions/ 12.5l/min newer versions
...
var torque2_pct = props.globals.getNode("sim/model/ec135/torque2-pct", 1);
var torque3_pct = props.globals.getNode("sim/model/ec135/torque-oei", 1);
...
var Engine = {
	...
	update: func(dt, trim = 0) {
		...
		# constant 75 kg/h for now (both turbines)
	...
	pos: { cutoff: 0, idle: 0.74, flight: 1 },
};
...
var engines = {
	...
	var n2trimfunction = props.globals.getNode("/controls/engines/n2trimfunction", 1);
	var aspfactor_test = props.globals.getNode("/controls/engines/aspfactor", 1);
	var pafactor_test = props.globals.getNode("/controls/engines/pafactor", 1);
	var trimvalue = me.trimN.getValue();
	var catabtn = props.globals.getNode("/controls/rotor/cata").getValue() or 0;
	var asp = props.globals.getNode("/instrumentation/airspeed-indicator/true-speed-kt").getValue() or 0;
	var aspfactor  = asp *0.00006896551;
	var pa = props.globals.getNode("/instrumentation/altimeter/pressure-alt-ft").getValue() or 0;
	var fcpa = pa - 4000;
	var pafactor = (fcpa / 166666);
	#var reln2 = trimvalue + 0.01;
	
	#rotorspeed control depending on CAT A button and density altitude
	
	
	
	if (asp >0){
	n2trimfunction.setValue(aspfactor);
	aspfactor_test.setValue (aspfactor);
	}
	if ((asp >0) and (pa > 4000)){
	n2trimfunction.setValue(aspfactor + pafactor);
	pafactor_test.setValue(pafactor);
	}elsif (catabtn == 1){
	n2trimfunction.setValue(aspfactor + 0.03);
	}elsif ((asp >0) and (catabtn == 1)){
	n2trimfunction.setValue(aspfactor + 0.03);
	}elsif ((asp >0) and (pa>4000)and (catabtn == 1)){
	n2trimfunction.setValue(aspfactor + pafactor + 0.03);
	}elsif (trimvalue > 0.04){
	n2trimfunction.setValue(0.04);
	}
	#else{
	#n2trimfunction.setValue(0);
	#}
		...
		var trim = (me.trimN.getValue() * 0.1) + n2trimfunction.getValue();
...
var procedure = {
	...
	process: func(id) {
		...
		#var battery = props.globals.getNode("/controls/electrical/battery_switch", 1);
		#var gen1 = props.globals.getNode("controls/electrical/engine/generator", 1);
		...
		if (me.stage == 1) {
			...
			setprop("controls/switches/fuel/transfer-pump[0]", "false");
			setprop("controls/switches/fuel/transfer-pump[1]", "false");
			setprop("controls/fuel/tank[2]/prime-pump", "true");
			setprop("controls/fuel/tank[1]/prime-pump", "true");
			cprint("", "1: press start button #1 -> spool up turbine #1 to N1 8.6--15%");
			setprop("/controls/rotor/brake", 0);
			setprop("/controls/engines/engine/fadec[0]/engine-state", "1");
			me.next(4);
			...
			setprop("controls/engines/engine[0]/power", "0.14");
			me.next(4.5);
			
		} elsif (me.stage == 3) {
			cprint("", "4: move power lever #1 to idle position -> engine #1 spools up to N1 63%");
			setprop("controls/engines/engine[0]/power", "0.74");
			me.next(2);
			...
			cprint("", "5: release start button #1\n");
			setprop("/controls/engines/engine[1]/fadec/engine-state", "1");
			me.next(4);
		
		} elsif (me.stage == 7) {
			cprint("", "2: move power lever #1 forward -> fuel injection");
			setprop("controls/engines/engine[1]/power", "0.14");
			me.next(2.5);
			...
			cprint("", "4: move power lever #1 to idle position -> engine #1 spools up to N1 63%");
			setprop("controls/engines/engine[1]/power", "0.74");
			me.next(2);
			...
			cprint("", "4: move power lever #1 to idle position -> engine #1 spools up to N1 63%");
			setprop("controls/engines/engine[0]/power", "1.0");
			setprop("controls/engines/engine[1]/power", "1.0");
			me.next(2);
			
		} elsif (me.stage == 10) {
			cprint("", "4: move power lever #1 to idle position -> engine #1 spools up to N1 63%");
			setprop("controls/switches/fuel/transfer-pump[0]", "true");
			setprop("controls/switches/fuel/transfer-pump[1]", "true");
			setprop("controls/fuel/tank[2]/prime-pump", "false");
			setprop("controls/fuel/tank[1]/prime-pump", "false");
			me.next(2);
...
var pendulum = props.globals.getNode("/sim/model/ec135/absorber-angle-deg", 1);
...
var Skid = {
	new: func(n) {
		var m = { parents: [Skid] };
		var soundN = props.globals.getNode("sim/model/ec135/sound", 1).getChild("slide", n, 1);
...
var volume_internal = props.globals.getNode("sim/model/ec135/sound/volume_internal", 1);
var volume_external = props.globals.getNode("sim/model/ec135/sound/volume_external", 1);
...
var crash = func {
	if (arg[0]) {
		# crash
		setprop("sim/model/ec135/tail-angle-deg", 35);
		setprop("sim/model/ec135/shadow", 0);
		setprop("sim/model/ec135/doors/door[0]/position-norm", 0.2);
		setprop("sim/model/ec135/doors/door[1]/position-norm", 0.9);
		setprop("sim/model/ec135/doors/door[2]/position-norm", 0.2);
		setprop("sim/model/ec135/doors/door[3]/position-norm", 0.6);
		setprop("sim/model/ec135/doors/door[4]/position-norm", 0.1);
		setprop("sim/model/ec135/doors/door[5]/position-norm", 0.05);
		...
		# uncrash (for replay)
		setprop("sim/model/ec135/tail-angle-deg", 0);
		setprop("sim/model/ec135/shadow", 1);
		doors.reset();
...
var doors = Doors.new();
#var config_dialog = gui.Dialog.new("/sim/gui/dialogs/ec135/config/dialog", "Aircraft/ec135/Dialogs/config.xml");
